...
impl<T: Fn(f64) -> f64> Transformer<T> {
    pub fn new(f: Box<dyn DensityFunction>, transform: T) -> Box<dyn DensityFunction> {
...
impl<T: Fn(f64) -> f64> DensityFunction for Transformer<T> {
