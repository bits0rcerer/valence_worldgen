...
const DEFAULT_SCRAMBLER: InputScrambler =
    |pos: BlockPos| i32x4::from_array([pos.x, pos.y, pos.z, 0]).cast();
...
impl DensityFunction for Noise {
    fn compute(&self, pos: BlockPos) -> f64 {
        ...
        let input = match self.shift.as_ref() {
            Shift::None => input,
            Shift::Dynamic { x, y, z } => {
                input + f64x4::from_array([x.compute(pos), y.compute(pos), z.compute(pos), 0.0])
            }
        };
