...
impl PerlinNoise {
    pub fn new(r: &mut dyn RandomSource, first_octave: i32, amplitudes: &[f64]) -> Self {
        ...
        let noise_levels = amplitudes.iter().enumerate().map(|(i, &amp)| {
            if amp != 0.0 {
                Some(ImprovedNoise::new(random_factory.with_hash_of(format!("octave_{}", first_octave + i as i32).as_str()).as_mut()))
            } else { None }
        }).collect::<Vec<_>>();

        let lowest_freq_input_factor = 2.0f64.powi(first_octave);
        let lowest_freq_value_factor = 2.0f64.powi((amplitudes.len() - 1) as i32) / (2.0f64.powi(amplitudes.len() as i32) - 1.0);
